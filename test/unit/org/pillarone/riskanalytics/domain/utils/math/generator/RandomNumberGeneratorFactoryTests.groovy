...
class RandomNumberGeneratorFactoryTests extends GroovyTestCase {
    ...
    void testException() {
        shouldFail {
            RandomNumberGeneratorFactory.getGenerator(
                    DistributionType.getStrategy(DistributionType.LOGNORMAL, ["mean": -1, "stDev": 5]))    //negative mean is illegal
    ...
    */
    void testlogNormalDistribution() {
        ...
        double empiricalMean = MathUtils.calculateMean(temp) //works only with array of double
        double empiricalVar = Math.pow(MathUtils.calculateStandardDeviation(temp), 2d)
