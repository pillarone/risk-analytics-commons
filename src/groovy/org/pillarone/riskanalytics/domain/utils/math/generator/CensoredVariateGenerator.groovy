...
/**
 * Given lower (left) and upper (right) bounds, min & max, CensoredVariateGenerator "wraps" the nextDouble method
 ...
 */
class CensoredVariateGenerator extends RandomVariateGen {
    double lowerBound, upperBound, shift
    RandomVariateGen generator

    public CensoredVariateGenerator(RandomStream randomStream, Distribution distribution, double min, double max, double shift = 0) {
        lowerBound = min
        upperBound = max
        this.shift = shift
