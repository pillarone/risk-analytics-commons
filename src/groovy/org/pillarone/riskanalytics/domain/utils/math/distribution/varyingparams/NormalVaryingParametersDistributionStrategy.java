 ...
 */
public class NormalVaryingParametersDistributionStrategy extends AbstractVaryingParameterDistributionStrategy {

    private ConstrainedMultiDimensionalParameter meanAndStDev = new ConstrainedMultiDimensionalParameter(
            GroovyUtils.convertToListOfList(new Object[]{1, 0d, 1d}), Arrays.asList(PERIOD, DistributionParams.MEAN, DistributionParams.STDEV),
            ConstraintsFactory.getConstraints(PeriodDistributionsConstraints.IDENTIFIER));

    public IParameterObjectClassifier getType() {
        return VaryingParametersDistributionType.NORMAL;
    }
    ...
    protected TreeMap<Integer, RandomDistribution> initDistributions() {
        ...
        for (int row = meanAndStDev.getTitleRowCount(); row < meanAndStDev.getRowCount(); row++) {
            ...
            distributionPerPeriod.put(period, DistributionType.getStrategy(DistributionType.NORMAL, ArrayUtils.toMap(
